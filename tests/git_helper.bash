...
#   --branch BRANCH: Override the default Git branch name (currently main)
#
# Output:
...
git_init() {
  ...
  while test $# -gt 0; do
    ...
    --*)
      echo "Unrecognized option '$1' passed to git_init" |
  ...
  local -r template="$BATS_TMPDIR/${BATS_TEST_FILENAME##*/}-$BATS_TEST_NAME.$BATS_TEST_NUMBER.XXXXXXXXXX"
  ...
  local output
  if ! output="$(git -C "$dir" init ${bare:+--bare} --initial-branch="$branch" 2>&1)"; then
    echo "$output" |
      batslib_decorate "ERROR: git_init" |
      fail
    return $?
